...
make_packages() {
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages.{both,xfce})" install
}
...
# Build ISO
make_iso() {
    out_filename="namib-xfce-$(date +%y%m)_x86_64.iso"
